...
class DirectBufferAccess {
    ...
    static VarHandle addressVarHandle;
    static {
        try {
            ...
            try {
                // 获取 Buffer 类中的 "address" 字段
                Field addressField = Buffer.class.getDeclaredField("address");

                // 使用 MethodHandles.lookup() 和 findVarHandle 获取 VarHandle
                addressVarHandle = MethodHandles.lookup()
                        .in(Buffer.class)
                        .findVarHandle(Buffer.class, "address", long.class);

            } catch (NoSuchFieldException | IllegalAccessException e) {
                throw new RuntimeException("Unable to get VarHandle for address field", e);
            }
        } catch (Exception e) {
    ...
    static long getAddress(Buffer buffer) {
        return (long) addressVarHandle.get(buffer);  // 使用 VarHandle 获取字段值
    }
